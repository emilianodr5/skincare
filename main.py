...
class ThirdPage(Screen): #opens fourth page when user clicks 'search an ingredient'
    def __init__(self, **kwargs):
        ...
        # back button template for all pages, trial on search page as it is currently the most complete
        self.add_widget(Button(text='<-', on_press=self.go_to_first_page, size_hint=(.2, .1),
                               pos_hint={'x': .25, 'y': .9}))
    ...
    def update_string(self, instance): #starts new protocol once user presses enter
        ...
        if len(results) == 0:
            ...
            #
            # add "did you mean" result; connect to request an ingredient page
        else:
            for row in results:
                R = row
...
class FourthPage(Screen): #opens fourth page when user clicks 'how our rating system works'
    def __init__(self, **kwargs):
        ...
        self.img = Image(source='scoring_explained.png')
